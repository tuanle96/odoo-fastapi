...
def x_timezone(
    x_timezone: Annotated[
        ...
        Header(
            alias="X-Timezone",
            description="The X-Timezone header is used to specify the timezone "
            "of the content to be returned. If a timezone is not available, the "
            "server will return the content in the default timezone.",
        ),
    ] = None,
) -> str:
    """This dependency is used at application level to document the way the timezone
    to use for the response is specified. The header is processed outside of the
    fastapi app to initialize the odoo environment with the right timezone.
    """
    return x_timezone
